...
function favListString(){var base=0;var bits=[];var list=Object.keys(favlist).sort();for(i=0;i<list.length;i++){if(isNaN(list[i]))continue;var toPush;if(base===0){base=list[i];toPush=base;}else{toPush=list[i]-base;}
bits.push(toPush);}
...
function initStars(){var useStarHint=typeof starHint!=='undefined';$tocStars=$(tocStarSelector).addClass('v');$tocStars.each(function(){var $this=$(this);var pid=pID||$this.parents('p.row').data('pid');$this.toggleClass('fav',favlist[pid]===1).data('pid',pid);if(useStarHint){$this.attr('title',starHint);}});$('h4 .star').off('click.star').on('click.star',function(){if($(this).hasClass('fav')){$tocStars.click();}else{$tocStars.not(".fav").click();}});$tocStars.off('click.star').on('click.star',function(){toggleFav($(this));});$('h4 button.delfaves').off('click.star').on('click.star',function(){favlist={};syncState();});syncState();}
...
function syncState(){try{$.totalStorage('cl_fav',favListString());}catch(e){}
updateFavCount();updateHeaderStar();}
